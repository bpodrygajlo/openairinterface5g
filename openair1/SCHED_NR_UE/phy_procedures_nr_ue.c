...
void phy_procedures_nrUE_TX_step1(PHY_VARS_NR_UE *ue, const UE_nr_rxtx_proc_t *proc, nr_phy_data_tx_t *phy_data, rate_match_info_uci_t *rm_info, unsigned int *G)
{
  const int slot_tx = proc->nr_slot_tx;
  const int frame_tx = proc->frame_tx;

  nr_ue_ulsch_procedures_step1(ue, frame_tx, slot_tx, phy_data, rm_info, G);
}

void phy_procedures_nrUE_TX_step2(PHY_VARS_NR_UE *ue, const UE_nr_rxtx_proc_t *proc, nr_phy_data_tx_t *phy_data, c16_t **txdataF, bool was_symbol_used[NR_SYMBOLS_PER_SLOT], rate_match_info_uci_t *rm_info, unsigned int G, c16_t **txp)
{
  const int slot_tx = proc->nr_slot_tx;
  const int frame_tx = proc->frame_tx;
  ...
  LOG_D(PHY, "****** start TX-Chain step 2 for AbsSubframe %d.%d ******\n", frame_tx, slot_tx);

  start_meas_nr_ue_phy(ue, PHY_PROC_TX);

  nr_ue_ulsch_procedures_step2(ue, frame_tx, slot_tx, phy_data, txdataF, was_symbol_used, rm_info, G);
  ...
  LOG_D(PHY, "Sending Uplink data step 2\n");
  ...
  LOG_D(PHY, "****** end TX-Chain step 2 for AbsSubframe %d.%d ******\n", proc->frame_tx, proc->nr_slot_tx);
                                  ...
                                  freq_alloc_bitmap_t *freq_alloc)
{
  ...
  c16_t (*rxdataF_comp)[NR_MAX_NB_LAYERS][rx_size_max] = (c16_t (*)[NR_MAX_NB_LAYERS][rx_size_max])scratch->rxdataF_comp;
  c16_t (*dl_ch_mag)[NR_MAX_NB_LAYERS][rx_size_max]    = (c16_t (*)[NR_MAX_NB_LAYERS][rx_size_max])scratch->dl_ch_mag;
  c16_t (*dl_ch_magb)[NR_MAX_NB_LAYERS][rx_size_max]   = (c16_t (*)[NR_MAX_NB_LAYERS][rx_size_max])scratch->dl_ch_magb;
  c16_t (*dl_ch_magr)[NR_MAX_NB_LAYERS][rx_size_max]   = (c16_t (*)[NR_MAX_NB_LAYERS][rx_size_max])scratch->dl_ch_magr;
  c16_t (*rho_dl)[NR_MAX_NB_LAYERS * NR_MAX_NB_LAYERS][rx_size_max] = (c16_t (*)[NR_MAX_NB_LAYERS * NR_MAX_NB_LAYERS][rx_size_max])scratch->rho_dl;
