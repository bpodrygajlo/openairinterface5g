                                    ...
                                    int N_TA_offset)
{
  ...
  for (int rep = 0; rep < params.reps; rep++) {
    rx_nr_prach_ru_internal_rep(p, beam_id, rxdata, fp, N_TA_offset, rep, &params, rxsigF_tmp);
  }
                        ...
                        c16_t (*rxsigF)[NR_PRACH_SEQ_LEN_L])
{
  ...
  int beam_id = p->beams[prachOccasion];
  prach_ru_params_t params = get_prach_ru_params(p, prachStartSymbol, fp);
  rx_nr_prach_ru_internal_rep(p, beam_id, rxdata, fp, N_TA_offset, rep, &params, rxsigF);
}
