...
void phy_init_nr_gNB(PHY_VARS_gNB *gNB)
{
  ...
  nfapi_nr_dbt_pdu_t *dbt_config = &cfg->dbt_config;
  if (dbt_config->num_dig_beams > 0) {
    LOG_I(NR_PHY, "L1 received DBT table with %d beams for %d baseband ports\n", dbt_config->num_dig_beams, dbt_config->num_txrus);
    if (dbt_config->num_txrus != Ptx)
      LOG_W(NR_PHY, "mismatch of %d TX antenna and %d BB ports in DBT table\n", Ptx, dbt_config->num_txrus);
    if (dbt_config->num_txrus != Prx)
      LOG_W(NR_PHY, "mismatch of %d RX antennas and %d BB ports in DBT table\n", Prx, dbt_config->num_txrus);
  }
