...
int nr_ue_ul_indication_step2(nr_uplink_indication_t *ul_info)
{
  module_id_t module_id = ul_info->module_id;
  NR_UE_MAC_INST_t *mac = get_mac_inst(module_id);
  int ret = pthread_mutex_lock(&mac->if_mutex);
  AssertFatal(!ret, "mutex failed %d\n", ret);
  LOG_D(PHY, "Locked in ul step 2, slot %d\n", ul_info->slot);

  if (is_ul_slot(ul_info->slot, &mac->frame_structure))
    nr_ue_ul_scheduler_step2(mac, ul_info);
  ret = pthread_mutex_unlock(&mac->if_mutex);
  AssertFatal(!ret, "mutex failed %d\n", ret);
  return 0;
}
...
nr_ue_if_module_t *nr_ue_if_module_init(uint32_t module_id)
{
  if (nr_ue_if_module_inst[module_id] == NULL) {
    ...
    nr_ue_if_module_inst[module_id]->ul_indication_step2 = nr_ue_ul_indication_step2;
