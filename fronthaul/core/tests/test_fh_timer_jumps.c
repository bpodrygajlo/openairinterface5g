...
int main(int argc, char **argv)
{
  ...
  if (tracker.symbols[0] != rte_atomic64_read(&timer.s_abs)) {
    fprintf(stderr, "FAIL: callback symbol %lu does not match current timer symbol %lu\n", tracker.symbols[0], rte_atomic64_read(&timer.s_abs));
    return 1;
  }
  ...
  printf("Test Case 4 passed: Backward jump (PTP sync backward) handling works perfectly.\n");
  ...
  uint64_t expected_sym = (uint64_t)((((int128)1718000000 - GPS_EPOCH_OFFSET_UNIX + GPS_LEAP_SECONDS) * NS_PER_SEC + 500000) * 28 / 1000000);
